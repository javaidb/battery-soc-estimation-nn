...
class ConfigAccessor:
    """
    A class that provides attribute-style access to nested configuration data.
    Automatically converts dictionaries to objects with attribute access.
    """
    def __init__(self, config_path='config/data_types.json'):
        with open(config_path, 'r') as file:
            self._config = json.load(file)
            
        # Convert the configuration to nested SimpleNamespace objects
        self.config = self._dict_to_namespace(self._config)
    ...
    def _dict_to_namespace(self, d):
        ...
        # Add attributes to the namespace
        for key, value in d.items():
            if isinstance(value, dict):
                setattr(ns, key, self._dict_to_namespace(value))
            elif isinstance(value, list):
                setattr(ns, key, [self._dict_to_namespace(item) if isinstance(item, dict) else item for item in value])
            else:
                setattr(ns, key, value)
                
        return ns
    
    def get_columns(self, data_label):
        """Get column configuration for a specific data label."""
        label_config = getattr(self.config.data_labels, data_label)
        return getattr(label_config, "column_names")
...
def import_data_config(data_label):
    """Legacy function for backwards compatibility."""
    return config.config.data_labels.__dict__[data_label]